    ...
    <div style={{ width:220, background: COLORS.primaryDark, minHeight:"100vh", display:"flex", flexDirection:"column", flexShrink:0 }}>
      <div style={{ padding:"20px 16px 16px", borderBottom:"1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <div style={{ background:COLORS.gold, borderRadius:6, padding:"3px 6px" }}><Icon name="shield" size={16} color={COLORS.primaryDark} /></div>
          <div>
      ...
      <div style={{ padding:"12px 8px", flex:1 }}>
        {navItems.map(item => (
          <button key={item.id} onClick={() => setActivePage(item.id)} style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:"9px 12px", borderRadius:8, border:"none", background: activePage===item.id ? "rgba(255,255,255,0.12)" : "transparent", color: activePage===item.id ? "#fff" : "rgba(255,255,255,0.6)", fontFamily:"inherit", fontSize:13, fontWeight: activePage===item.id ? 600 : 400, cursor:"pointer", marginBottom:2, textAlign:"left" }}>
      ...
      <div style={{ padding:"12px 8px", borderTop:"1px solid rgba(255,255,255,0.08)" }}>
        <button onClick={() => { logout(); setView("home"); }} style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:"9px 12px", borderRadius:8, border:"none", background:"transparent", color:"rgba(255,255,255,0.5)", fontFamily:"inherit", fontSize:13, cursor:"pointer" }}>
